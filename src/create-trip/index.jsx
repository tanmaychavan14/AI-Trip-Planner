...
function CreateTrip() {
  ...
  const OnGenerateTrip = async () => {
    ...
    // Improved validation
    if (!formData?.location?.label || !formData?.noOfDays || !formData?.budget || !formData?.people) {
      toast.error("Please ensure all questions are answered!");
      return;
    }

    if (formData.noOfDays > 20) {
      toast.error("Trip days cannot exceed 20!");
      return;
    }
